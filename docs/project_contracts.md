...
- `calculate_post_ffe_imp()` 吃 selected pre-DTE impairment、selected DTE result、oversampled victim pulse `h`，負責 FFE/DFE 後的 finalization；在 all-stage shared `status.imp.eq_ch` 建立 `h_w/h_XTs_w/h_ISI/h_w_J`，在 `status.imp.post_ffe.psd` 建立 `A_s/sigma_ISI/S_jn_RJ/S_gn_adc`，在 `status.imp.post_ffe.adc_input` 建立 final DFE COM 所需的 exact ADC-input PMF material。它同時把 raw `COMDTEStatus.w_lim` 轉成 post-FFE grid 的 `status.imp.H_rxffe: SampledResponse`；`status.imp.w_lim` 是 read-only proxy。`sigma_ISI` 是 scalar impairment，明確屬於 PSD status，不屬於 equivalent-channel sequence。
...
- 178A quantization contract 分成兩層：runtime profile 的 `pre_dte_pmf_method` 只控制 pre-DTE `S_qn`，供 MMSE 建立 `S_total/R_n`；選定 pos 後，`calculate_post_ffe_imp()` 一律建立 `method="pmf_exact"` 的 ADC-input PMF material，保存 `p_sig/p_s/p_XT/p_DD/p_ga/p_n/p_sn/V_qc/delta` 至 `status.imp.post_ffe.adc_input`。`status.imp.post_ffe.psd` 同時保存 ADC-input Gaussian PSD `S_gn_adc=S_rn+S_tn+S_jn_RJ` 與 `sigma_gn_adc`。`calculate_COM_DFE()` 只讀取此 exact `delta` 建立並 FFE-filter `p_qn`，不重算 `V_qc`；final `COMPMFStatus.p_qn` 保存此分布供 export/report。
...
- 若 DTE 有 floating tap gap，`COMDTEStatus.w_lim` 仍是固定長度的 raw zero-filled FIR vector；`_build_dte_status()` 已完成 scatter。post-FFE stage 只在建立 `h_w`、`h_XTs_w`、`h_w_J` 與 post-grid PSD filtering 時轉成 `H_rxffe: SampledResponse`；MMSE/PMF FIR 計算與 coefficient report 一律使用 raw vector，不能讀 zero-padded `H_rxffe.ir`。
...
- COM DFE 的 Gaussian noise PMF 直接取 post-FFE `S_gn_adc`；不可再以 pre-DTE grid 的 FFE response 重複 filtering。
